...
class TopUpViaPosRis(http.Controller):
    ...
    def isipulsa(self, phone, id, pin, kode, trx_id, counter, trx_type):
        payload={}
        headers = {}
        ...
        if len(trx_type) > 0:
            url = "http://103.119.55.59:8080/api/h2h?id="+str(id)+"&pin="+str(pin)+"&user="+str(user)+"&pass="+str(password)+"&kodeproduk="+str(product_code)+"&tujuan="+str(phone)+"&counter="+counter+"&idtrx="+str(trx_id)+"&jenis="+str(trx_type)
        else:
            url = "http://103.119.55.59:8080/api/h2h?id="+str(id)+"&pin="+str(pin)+"&user="+str(user)+"&pass="+str(password)+"&kodeproduk="+str(product_code)+"&tujuan="+str(phone)+"&counter="+counter+"&idtrx="+str(trx_id)
